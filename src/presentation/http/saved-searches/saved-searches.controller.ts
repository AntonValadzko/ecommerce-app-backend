...
export class SavedSearchesController {
  constructor(
    ...
    private readonly presenter: SavedSearchesPresenter,
  ) {}
  ...
  async findAll(@SessionId() sessionId: string) {
    return this.presenter.toDataResponse(await this.savedSearchesService.list(sessionId));
  }
  ...
  async create(@SessionId() sessionId: string, @Body() dto: CreateSavedSearchDto) {
    const saved = await this.savedSearchesService.save(
      sessionId,
      dto.name,
      dto.query.toProductQuery(),
    );
    return this.presenter.toDataResponse(saved);
  }
